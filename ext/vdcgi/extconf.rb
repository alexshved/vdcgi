...
def add_ssl_defines(header)
  all_modes_found = %w[SSL_MODE_DISABLED SSL_MODE_PREFERRED SSL_MODE_REQUIRED SSL_MODE_VERIFY_CA
                       SSL_MODE_VERIFY_IDENTITY].inject(true) do |m, ssl_mode|
...
if with_config('openssl-dir')
  ...
  if lib
    ...
    abort "-----\nCannot find library dir(s) #{lib}\n-----" unless lib && lib.split(File::PATH_SEPARATOR).any? { |dir|
      File.directory?(dir)
    }
...
if inc && lib
  ...
  abort "-----\nCannot find include dir(s) #{inc}\n-----" unless inc && inc.split(File::PATH_SEPARATOR).any? { |dir|
    File.directory?(dir)
  }
  abort "-----\nCannot find library dir(s) #{lib}\n-----" unless lib && lib.split(File::PATH_SEPARATOR).any? { |dir|
    File.directory?(dir)
    ...
    if (libdir = rpath_dir[%r{(-L)?(/[^ ]+)}, 2])
      ...
      if RbConfig::CONFIG["RPATHFLAG"].to_s.empty? && try_link('int main() {return 0;}', rpath_flags)
        ...
        if RbConfig::CONFIG["RPATHFLAG"].to_s.empty?
          ...
          warn "\nDon't know how to set rpath on your system, if MySQL libraries are not in path may not load\n"
        end
